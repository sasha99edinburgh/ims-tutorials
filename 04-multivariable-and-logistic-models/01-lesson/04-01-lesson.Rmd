---
title: "Multivariate and Logistic Models: 1 - Parallel Slopes"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
```{r ex1-check}
grade_result(
  pass_if(~identical(floor(.result$Coefficients[1]), 37), "You have fitted the model correctly!"),
  fail_if(~identical(round(.result$Coefficients[1], 1), 38.4), "Did you forget to include the `cond` variable in your model?"),
   fail_if(~identical(floor(.result$Coefficients[1]), 53), "Did you forget to include the `wheels` variable in your model?"),
...
```{r ex2-plot, exercise = TRUE, exercise.setup = "ex2-setup"}
...
```{r ex3-check}
grade_result(
  pass_if(~identical(floor(.result$Coefficients[1])), 118, "You have fitted the model correctly!"),
  fail_if(~identical(floor(.result$Coefficients$[1]), 116), "Did you forget to include the variable `parity`?"),
  fail_if(~identical(floor(.result$Coefficients$[1]), 120), "Did you forget to include the variable `age`?"),
  fail_if(~TRUE, "Look at the hints for help.")
)
...
```{r ex4-check}
grade_result(
  pass_if(~identical(round(.result$Coefficients[1], 2), -0.93), "You have fitted the model correctly!"),
  fail_if(~identical(floor(.result$Coefficients$[1]), -11), "Did you forget to include the variable `smoke`?"),
  fail_if(~identical(floor(.result$Coefficients$[1]), 123), "Did you forget to include the variable `age`?"),
  fail_if(~TRUE, "Look at the hints for help.")
)
