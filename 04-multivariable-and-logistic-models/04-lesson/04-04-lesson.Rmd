---
title: "Multivariate and Logistic Models: 4 - Logistic Regression"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
*Hint:* In `geom_smooth()` define the `method` and `se` arguments for a straight line fit without the standard error band.
...
```{r ex2-2-hint-1}
It's pretty similar to the first exercise.
```

```{r ex2-2-hint-2}
# scatterplot with jitter
data_space <- ggplot(data = MedGPA_middle, aes(y = ___, x = ___)) + 
  geom_jitter(width = 0, height = 0.05, alpha = 0.5)
...
```{r ex3-check}
grade_code("You have written the model correctly!")
```
...
```{r ex5-1-hint-2}
data_space <- ggplot(data = ___, aes(x = ___, y = ___)) + 
  geom_point() + 
  geom_line()
```
...
```{r ex5-1-check}
grade_code("You solution is correct!")
```

```{r ex5-2, exercise=TRUE, exercise.setup = "ex5-1-setup"}
# augmented model
MedGPA_plus <- ___
```
...
data_space +
  ____(data = ___, aes(___), color = "red")
```

```{r ex5-3-hint}
data_space +
  geom_line(data = ___, aes(x = GPA, y = ___), color = "red")
```
...
```{r ex5-3-check}
grade_code("Your plot is correct!")
```
...
```{r ex8-hint-2}
augment(___, newdata = ___, type.predict = "response")
```
...
```{r ex8-check}
grade_code("You have predicted the value correctly.")
```
