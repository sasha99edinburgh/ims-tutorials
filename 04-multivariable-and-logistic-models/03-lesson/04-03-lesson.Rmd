---
title: "Multivariate and Logistic Models: 3 - Multiple Regression"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
```{r ex1-check}
grade_result(
  pass_if(~identical(floor(.result$coefficients[[1]]),51), "You have written the model correctly!"),
  fail_if(~TRUE, "Make sure you are using the correct variable names. Use '?mariokart' to identify the variable names.")
)
```
...
```{r ex2-1-hint}
price_hats <- augment(___, newdata = ___)
```
...
```{r ex4, exercise=TRUE}
# Fit the model using start_pr and duration

```

```{r ex4-hint}
# Fit the model using start_pr duration
lm(total_pr ~ start_pr + duration, data = mariokart)
```
