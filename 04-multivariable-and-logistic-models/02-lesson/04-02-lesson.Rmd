---
title: "Multivariate and Logistic Models: 2 - Evaluating and extending parallel slopes model"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
```{r ex1-1, exercise=TRUE, exercise.setup="ex1-mod-setup"}
...
# set the seed to 1 then add random noise
set.seed(___)
...
```{r ex1-3, exercise=TRUE, exercise.setup = "ex1-mod2-setup"}
# compute new model
mod2 <- ____
```
...
```{r ex1-4,exercise=TRUE, exercise.setup="ex1-mod2-setup"}
...
```{r ex1-4-check}
grade_result(
  pass_if(~identical(round(.result$r.squared,3),0.727), "Your new R^2 and adjusted R^2 are correct!"),
...
```{r ex3-check}
grade_result(
  pass_if(~identical(round(.result$coefficients[[4]],1),2.3), "You have fit the model correctly!"),
  fail_if(~identical(round(.result$coefficients[[4]],2),0.36), "Did you forget to include the term `duration` in your model?"),
  fail_if(~identical(floor(.result$coefficients[[1]]),51), "Did you forget to include the term `cond` in your model?"),
...
```{r ex4-check}
grade_code("You have produced the correct plot!")
```
