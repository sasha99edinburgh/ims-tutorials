---
title: "Multivariate and Logistic Models: 5 - Case Study: Italian restaurants in NYC"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
*Hint:* The `pairs()` function is helpful for looking for pairs of variables that have high correlation.
...
```{r ex2-1-hint-2}
#Price by Food plot
ggplot(data = nyc, aes(x = ____, y = ____)) +
  geom_point()
```
...
```{r ex2-2-solution}
# Price by Food model
lm(Food~Price, data = nyc)
```

```{r ex2-2-check}
grade_result(
  pass_if(~identical(floor(.result$coefficients[[1]]), -18), "You have fit the model correctly!"),
  fail_if(~identical(floor(.result$coefficients[[1]]),14), "Did you perhaps use `Food ~ Price`? Remember, it's y ~ x, not x ~ y"),
  fail_if(~TRUE, "Look at the hint for help.")
)
```
...
```{r ex4-1-check}
grade_result(
  pass_if(~identical(floor(.result$coefficients[[1]]), -22), "You have written the model correctly!"),
  fail_if(~TRUE, "Make sure you write the model using the correct variable names.")
)
```

```{r ex4-2, exercise = TRUE, exercise.setyp = "ex4-setup"}
# draw 3D scatterplot
p <- plot_ly(data = ___, z = ~___, x = ~___, y = ~___, opacity = 0.6) %>%
  add_markers() 

# draw a plane
p %>%
  _____(x = ~x, y = ~y, z = ~___, showscale = FALSE)
```

```{r ex4-2-hint-1}
# draw 3D scatterplot
p <- plot_ly(data = nyc, z = ~___, x = ~Food, y = ~___, opacity = 0.6) %>%
  add_markers() 
...
```{r ex6-check}
grade_code("Tour solution is correct!")
```
